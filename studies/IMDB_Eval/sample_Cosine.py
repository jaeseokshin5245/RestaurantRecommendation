...
# 현재 경로에서 source.csv 파일을 pandas 파일을 통해서 입력
menus = pd.read_csv('./source.csv')

# 해당 입력 파일에서 리스트에 해당하는 행(columns)를 입력 받음
# + 자동으로 csv의 행렬 또는 시트를 읽는 기능을 추가할 것
menus_df = menus[['업소명','대분류', '메뉴', '별점', '별점 수']]

# 행(columns)의 최대 길이를 100으로 한정
...
# menus_df['메뉴]의 내용을 위의 벡터 변환기에 넣어 변환 후 menu_mat에 대입
menu_mat = count_vec.fit_transform(menus_df['메뉴'])

# 메뉴 행렬 자기 자신에 대한 코사인 유사도 측정
menu_sim = cosine_similarity(menu_mat, menu_mat)

# "아키타"와 메뉴 유사도가 높은 식당 상위 5개에 대해 구함
